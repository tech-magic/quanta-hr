...
echo "Running first Ansible playbook, on remote AWS machine with GPUs -> $ANSIBLE_LOG_REMOTE_GPU"

REMOTE_GPU_PLAYBOOK="$ANSIBLE_DIR/remote/remote_gpu_playbook.yml"
if ansible-playbook -vvv "$REMOTE_GPU_PLAYBOOK" \
  -i "$REMOTE_INVENTORY_FILE" \
  -e "llm_settings=$LLM_SETTINGS_JSON" \
  --extra-vars "@$TF_VARS_JSON" | tee "$ANSIBLE_LOG_REMOTE_GPU"; then
    echo "✅ $REMOTE_GPU_PLAYBOOK completed successfully."

    # -------------------------------
    # Run Ansible Playbook on Local CPU Host with logging and retry logic
    # -------------------------------
    ANSIBLE_LOG_LOCAL_CPU="$TARGET_DIR/ansible_local.log"
    ...
    echo "Running second Ansible playbook with up to $MAX_RETRIES retries..."

    while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
        RETRY_COUNT=$((RETRY_COUNT + 1))
        echo "➡️  Attempt $RETRY_COUNT of $MAX_RETRIES..."

        if ansible-playbook "$LOCAL_CPU_PLAYBOOK" \
            -i "$LOCAL_INVENTORY_FILE" \
            -e "llm_settings=$LLM_SETTINGS_JSON"
            --extra-vars "@$TF_VARS_JSON" | tee -a "$ANSIBLE_LOG_LOCAL_CPU"; then
            echo "✅ $LOCAL_CPU_PLAYBOOK succeeded on attempt $RETRY_COUNT."
            SUCCESS=true
            break
        else
            echo "❌ $LOCAL_CPU_PLAYBOOK failed on attempt $RETRY_COUNT."
    ...
    if [ "$SUCCESS" = true ]; then
        ...
        echo "❌ $LOCAL_CPU_PLAYBOOK failed after $MAX_RETRIES attempts."
        exit 1
    fi

else
    echo "❌ First Ansible playbook failed. Skipping secondary playbook."
    exit 1
fi
